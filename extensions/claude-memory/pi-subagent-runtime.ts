...
async function resolveRuntimeModule(): Promise<RunPiSubagentModule> {
  ...
  if (!runtimePath) {
    throw new Error(
      'pi-subagent-tool is required but was not found in ~/.pi/agent. Install it with `pi install git:github.com/san-tian/pi-subagent-tool`.',
    );
  }
...
async function findRuntimePath(): Promise<string | null> {
  const runtimePath = path.join(getPiPackagesRoot(), "san-tian", "pi-subagent-tool", "extensions", "subagent", "runtime.ts");
  return (await exists(runtimePath)) ? runtimePath : null;
}
